    ...
    def draw_lines(self):
        for line in self.lines_coordinates:
            ...
            if start[0] == end[0]: # vertical line
                ...
                # either going up left, up right, down left, down right
                # plot[y][x]
                # if going up: 
                diagonal_points = abs(start[1] - end[1])
                print(abs(start[1] - end[1]) - abs(start[0] - end[0]))
                if start[0] > end[0]:
                    # if going right
                    if start[1] < end[1]:
                        # - +
                        for index in range(diagonal_points + 1):
                            self.plot_area[start[0] - index][start[1] + index] += 1
                    # if going left
                    if start[1] > end[1]:
                        # - -
                        for index in range(diagonal_points + 1):
